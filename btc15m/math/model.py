...
class SignalWeights:
    # These weights are manually calibrated priors, not estimated by an
    # optimizer. Positive weights favor UP, negative weights favor DOWN.
    bias: float = 0.0
    mom_30m: float = 0.55
    mom_60m: float = 0.75
    mom_120m: float = 0.35
...
FEATURE_SCALES: dict[str, float] = {
    # Each scale converts raw features into roughly comparable score units
    # before the logistic map. These are hand-set stabilizers, not fits.
    "mom_30m": 350.0,
    "mom_60m": 280.0,
    "mom_120m": 200.0,
    "vol_2h": 250.0,
...
def normalize_features(raw_features: dict[str, float]) -> dict[str, float]:
    ...
    for name, value in raw_features.items():
        ...
        # x_norm = clip(scale * x_raw, -6, 6) keeps any single feature from
        # dominating the logistic score because of one large outlier.
