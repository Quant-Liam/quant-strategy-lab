...
def _rsi(series: pd.Series, period: int = 14) -> pd.Series:
    # Wilder RSI uses exponentially smoothed gains and losses:
    # RSI_t = 100 - 100 / (1 + RS_t), where RS_t = avg_gain_t / avg_loss_t.
    delta = series.diff()
...
def compute_price_features(candles_15m: pd.DataFrame, live_price: float) -> dict[str, float]:
    ...
    # Most price transforms are expressed in log space so additive moves
    # correspond to percentage returns on BTC.
    log_close = np.log(closes)
    rets = log_close.diff().dropna()

    # Momentum windows are calibrated as 30m / 60m / 120m because they are
    # short enough for the 15m trading horizon but still capture structure.
    ...
    # vol_2h is the standard deviation of the last 8 x 15m log returns:
    # sigma_2h = std(r_{t-7}, ..., r_t).
    ...
    # These stalling thresholds are hand-set heuristics: strong 60m move but
    # muted 30m follow-through suggests exhaustion rather than acceleration.
...
def compute_market_features(
    ...
    # This stage is just feature assembly; it does not calibrate weights.
